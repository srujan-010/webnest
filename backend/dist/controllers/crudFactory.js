...
function createCrudController(Model, modelName, options) {
    const getAll = async (req, res) => {
        try {
            const { trash, page = 1, limit = 50, search } = req.query;
            const filter = trash === 'true'
                ? { isDeleted: true }
                : { isDeleted: { $ne: true } };
            if (search) {
                filter.$or = [
    ...
    const restore = async (req, res) => {
        try {
            const item = await Model.findByIdAndUpdate(req.params.id, { isDeleted: false, $unset: { deletedAt: '' } }, { new: true });
            if (!item)
                return res.status(404).json({ success: false, message: `${modelName} not found.` });
            await AuditLog_1.default.create({
    ...
    const publish = async (req, res) => {
        try {
            const item = await Model.findByIdAndUpdate(req.params.id, { isPublished: true, status: 'published', publishedAt: new Date() }, { new: true });
            if (!item)
                return res.status(404).json({ success: false, message: `${modelName} not found.` });
            // Trigger Next.js revalidation
            const revalidateUrl = `${process.env.CLIENT_URL || 'http://localhost:3000'}/api/revalidate`;
